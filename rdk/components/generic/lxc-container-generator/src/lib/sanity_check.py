...
class cSanityCheck(object):
    ...
    def __init__(self, filename, privileged, name, rootfsPath, confPath, givenTags, enableMountCheck, extRootfsList):
        self.name = name
        self.filename = filename
        self.privileged = privileged
        self.rootfsPath = rootfsPath
        # General tool configuration file
        self.confPath = confPath + "/config.ini"
        # Autoinstall libs configuration file
        self.autoInstallConfPath = confPath + "/libs_autoinstall.ini"
        # Dictionary with lxc config option names
        self.configOptionNamesDictPath = confPath + "/config_option_names.json"
        self.enableMountCheck = enableMountCheck
        self.autoInstallLibs = {}
        self.dirList = []
        self.tags = []
        self.rangeTags = []
        self.writableDirs = []
    ...
    def parseConfig(self):
        ...
        self.dbusSocket = config.get("DBUS", "SOCKET_ADDR")

    def sanitizeLibName(self, name):
        name = name.strip()
        if name == "":
            return False
        else:
    ...
    def parseAutoInstallLibs(self):
        cfg = configparser.ConfigParser()
        cfg.optionxform = str
        fileFound = True

        # Autoinstall libs are optional so lack of configuration file is OK
        if len(cfg.read(self.autoInstallConfPath)) == 0:
            dbg.logTrace(1, "*** WARNING: Autoinstall libraries config file not present [%s]" % str(self.autoInstallConfPath))
            fileFound = False

        # Try to find libraries for all categories
        for category in self.LibCategory:
            try:
                libs = cfg["AUTOINSTALL"][category.value].split("\n")
                libs = list(filter(self.sanitizeLibName, libs))
            except:
                if fileFound is True:
                    dbg.logTrace(1, "*** WARNING: No libs found for autoinstall category [%s]" % category.value)
                libs = []

            self.autoInstallLibs[category] = libs

    def getAutoInstallLibs(self, category):
        if not isinstance(category, self.LibCategory):
            raise Exception("[!!! ERROR !!!] Invalid autoinstall lib category [%s]" + str(category))
        return self.autoInstallLibs[category]
    ...
    def getPlatformSettingValue(self, name):
        ret = ""
        for entry in self.tags:
    ...
    def pathExist(self, path):
        ...
        if (not os.path.exists(self.rootfsPath + "/" + path) and not os.path.islink(self.rootfsPath + "/" + path)):
            if (self.extRootfsList is not None):
                for ext_rootfs_path in self.extRootfsList:
                    p = os.path.join(ext_rootfs_path + "/" + path)
    ...
    def report_skipping_entry(self, node, path):
        ...
        def c_default(node):
            v = "<" + node.tag
            for (an, av) in list(node.attrib.items()):
                v += " %s=\"%s\"" % (an, av)
            return v + ">"
        ...
        dbg.logTrace(1, "Skipping entry %s \t Current Settings: %s %s" % (entryName, self.getPlatformSettings(), messageFooter))
    ...
    def parseRange(self, rrange):
        ...
        if len(parts) == 2:
            ...
            raise Exception("[!!! ERROR !!!] Range stop value isn't set: %s " % (rrange))

        rrange = range(x, y)
        if len(rrange) == 0:
            raise Exception("[!!! ERROR !!!] Invalid range: %s " % (rrange))

        return x, y

    def readConfigOptionNamesDict(self):
        with open(self.configOptionNamesDictPath, "r") as dict_fd:
            data = dict_fd.read()
        self.confOptsDict = json.loads(data)
        if len(self.confOptsDict) == 0:
            raise Exception("[!!! ERROR !!!] Empty config option names dictionary: %s " % (self.configOptionNamesDictPath))
