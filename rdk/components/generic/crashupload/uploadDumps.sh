...
if [ "$DEVICE_TYPE" != "broadband" ] && [ "$DEVICE_TYPE" != "mediaclient" ] && [ -f $RDK_PATH/commonUtils.sh ]; then
...
processDumps()
{
    ...
    do
	    ...
	    #check for PrivacyModes Control
            if [ "$DEVICE_TYPE" = "mediaclient" ]; then
                privacyMode=`rbuscli getvalues Device.X_RDKCENTRAL-COM_UserSettings.PrivacyModeChanged | grep "Value" | cut -d":" -f 2 | cut -d" " -f 2`
