...
debug_log_msg() {
    for file in "$@"; do
        if [ ! -f "$file" ]; then
            log_msg "[DEBUG] $file does not exist"
            continue
        fi
        ...
        log_msg "[DEBUG] md5sum of $filename : $md5"
        log_msg "[DEBUG] sha256sum of $filename : $sha256"
        log_msg "[DEBUG] size of $filename : $size"
    done
}
...
if [ "$BUILD_TYPE" != "prod" ]; then
    log_msg "Enabling RDM OpenSSL Debugging"
    touch /tmp/debug_rdmopenssl
fi
...
if [ "$SIGNATURE_TYPE" = "kms" ];then
        ...
        if [ -f $CONFIGPARAMGEN ]; then
            ...
            log_msg "${CONFIGPARAMGEN} not found. Creating dummy key file"
            touch $KMS_INTERMEDIATE_RSA_KEY_FILE
        fi
...
if [ -f /usr/bin/opensslVerify ];then
      ...
      debug_log_msg "$CPEMANIFEST_PATH" "$WORKDIR/$SIGNATURE_FILE" "$KMS_INTERMEDIATE_RSA_KEY_FILE" "$WORKDIR/$PACKAGE_FILE"
