...
echo "`/bin/timestamp` [RFC]:: POSTPROCESSING IS RUN NOW !!!" >> $LOG_PATH/rfcscript.log
...
# PROCESSING BROADBAND SPECIFIC CALLS
    ...
    if [ ! -f /tmp/.rfcLock ] ; then
       ...
       if [ -f $RFC_SSH_FILE ] ; then
           echo "`/bin/timestamp` RFC File for SSH present. Refreshing Firewall" >> $LOG_PATH/dcmrfc.log
           #sh $RDK_PATH/rfc_refresh.sh SSH_REFRESH &
	   sysevent set firewall-restart
        else
           echo "`/bin/timestamp` RFC File for SSH is not present or empty" >> $LOG_PATH/dcmrfc.log
       fi
       dmcli eRT setv Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.Snmpv3DHKickstart.RFCUpdateDone bool true
       echo "`/bin/timestamp` [RFC]:: RFCUpdateDone set to true" >> $LOG_PATH/rfcscript.log
    else
       echo "`/bin/timestamp` /tmp/.rfcLock file present" >> $LOG_PATH/dcmrfc.log
    fi
else
...
echo "`/bin/timestamp` [RFC]:: POSTPROCESSING IS COMPLETE !!!" >> $LOG_PATH/rfcscript.log
