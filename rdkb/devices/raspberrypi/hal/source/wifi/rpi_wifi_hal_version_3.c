...
INT rpi_getRadioCapabilities(int radioIndex, wifi_radio_capabilities_t *rcap)
{
	...
	if (!strcmp(band, "2.4GHz"))
	{
		...
		rcap->channelWidth[radioIndex]= WIFI_CHANNELBANDWIDTH_20MHZ | WIFI_CHANNELBANDWIDTH_40MHZ;
	...
	else if (!strcmp(band, "5GHz"))
	{
		...
		rcap->channelWidth[radioIndex]= WIFI_CHANNELBANDWIDTH_20MHZ | WIFI_CHANNELBANDWIDTH_40MHZ | WIFI_CHANNELBANDWIDTH_80MHZ;
...
INT wifi_getRadioOperatingParameters(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
	...
	else if (!strcmp(buf, "8080MHz")) operationParam->channelWidth = WIFI_CHANNELBANDWIDTH_80_80MHZ;
	else
	{
...
INT wifi_setRadioOperatingParameters(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
	...
	if ((operationParam->channelWidth == 1) || (operationParam->channelWidth == 0))
	...
	else if (operationParam->channelWidth == 8) 
		strcpy(ChannelWidth,"160MHz");
	else if (operationParam->channelWidth == 10) 
	...
	len = sizeof(data_transmitrates);
        for (i=0; i<ARRAY_SZ(wifiDataTxRateMap); i++) {
                if(operationParam->operationalDataTransmitRates & wifiDataTxRateMap[i].DataTxRateEnum) {
                        if(data_transmitrates[0]== '\0')
                                size = snprintf(data_transmitrates, len, "%s", wifiDataTxRateMap[i].DataTxRateStr);
                        else
                                size += snprintf(data_transmitrates + size, len - size," %s",wifiDataTxRateMap[i].DataTxRateStr);
                }
        }
        if (wifi_setRadioOperationalDataTransmitRates(index,data_transmitrates) != RETURN_OK) {
                return RETURN_ERR;
        }
