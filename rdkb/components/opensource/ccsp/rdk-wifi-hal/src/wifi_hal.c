...
#if defined(VNTXER5_PORT)
        if(vap->vap_mode == wifi_vap_mode_ap && (radio->oper_param.variant & WIFI_80211_VARIANT_BE))
        {
            if( platform_recreate_interface(radio, vap, &interface, true) != RETURN_OK)
            {
                wifi_hal_error_print("%s:%d: vap index:%d platform_recreate_interface failed\n", __func__,
                                   __LINE__, vap->vap_index);
                continue;
            }
        }
