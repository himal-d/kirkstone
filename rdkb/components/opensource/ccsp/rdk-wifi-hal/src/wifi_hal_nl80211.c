...
int nl80211_update_wiphy(wifi_radio_info_t *radio)
{
    ...
    if ((radio->oper_param.channelWidth == WIFI_CHANNELBANDWIDTH_320MHZ) && (radio->oper_param.variant & WIFI_80211_VARIANT_BE)) {
        platform_config_eht_chanspec(radio->rdk_radio_index, &radio->oper_param);
        goto Exit;
    } else {
        platform_set_eht(radio->rdk_radio_index, (radio->oper_param.variant & WIFI_80211_VARIANT_BE) ? true : false);
    }
#endif
                               ...
                               int sec_chan_offset, int bandwidth, int bw, int cf1, int cf2)
{
    ...
    if(bandwidth == WIFI_CHANNELBANDWIDTH_160MHZ) {
        int Chan_width_80MHz = CHANWIDTH_80MHZ;
        hostapd_set_oper_chwidth(interface->u.ap.hapd.iconf, CHANWIDTH_80MHZ);
