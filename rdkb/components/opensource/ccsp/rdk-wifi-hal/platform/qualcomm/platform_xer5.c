...
static int del_interface_by_vap_index(unsigned int vap_index)
{
    ...
    for (i = 0; i < g_wifi_hal.num_radios; i++) {
        ...
        while (interface != NULL) {
            if (interface->vap_info.vap_index == vap_index) {
                wifi_hal_info_print("%s:%d: Removing vap_index:[%d] from g_wifi_hal.radio_infop[%d]\n",__func__, __LINE__, vap_index, i);
                hash_map_remove(radio->interface_map, interface->name);
                hash_map_destroy(interface->acl_map);
    ...
    wifi_hal_info_print("%s:%d: vap_index:[%d] has no interface info\n",__func__, __LINE__, vap_index);
    return 0;
}
...
int update_hostap_mlo(wifi_interface_info_t *interface) {
    ...
    if (!conf->disable_11be) {
        ...
        //mld_mac_addr
        os_memcpy(conf->mld_mac_addr, hapd->own_addr, ETH_ALEN);
...
static int configure_hostap_interface(wifi_radio_info_t *radio, wifi_interface_info_t *interface,
                              wifi_vap_info_t *vap)
{
    int filtermode;
    char mld_ifname[32];
...
int platform_set_radio_pre_init(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
    ...
    wifi_interface_info_t *new_intf = NULL;
    int existing_vap_indices = 0;
    int i;
    wifi_vap_info_t old_vap_info = {0};
    ...
    if(!radio->configured)
    {
        wifi_hal_info_print("%s:%d: Radio is getting configured for the first time. No need of vap re-creation\n",
                                                                                        __func__, __LINE__);
        return RETURN_OK;
    }
    ...
    for(i=0; i < MAX_INTERFACE_IDX; i++)
    {
        ...
        memcpy(&old_vap_info, &interface->vap_info, sizeof(wifi_vap_info_t));
        ...
        if(radio->oper_param.variant & WIFI_80211_VARIANT_BE && !(operationParam->variant & WIFI_80211_VARIANT_BE))
        {
            ...
            wifi_hal_error_print("%s:%d: 11BE mode is disabled. Recreate VAPs w/o MLD address. [%d -> %d]\n",
                                    __func__, __LINE__, radio->oper_param.variant, operationParam->variant);
            if(platform_recreate_interface(radio, &old_vap_info, &new_intf, false) != RETURN_OK)
            {
                wifi_hal_error_print("%s:%d: Failed to recreate VAP index %d without MLD address.\n",
                                                __func__, __LINE__, i);
                continue;
            }
        }
        else if(operationParam->variant & WIFI_80211_VARIANT_BE && !(radio->oper_param.variant & WIFI_80211_VARIANT_BE))
        {
            ...
            wifi_hal_error_print("%s:%d: 11BE mode is enabled. Recreate VAPs with MLD address. [%d -> %d]\n",
                                    __func__, __LINE__, radio->oper_param.variant, operationParam->variant);
            if(platform_recreate_interface(radio, &old_vap_info, &new_intf, true) != RETURN_OK)
            {
                wifi_hal_error_print("%s:%d: Failed to recreate VAP index %d with MLD address.\n",
                                                __func__, __LINE__, i);
                continue;
            }
        }
        ...
        {
            wifi_hal_dbg_print("%s:%d: Vap re-creation is not required!\n", __func__, __LINE__);
            return 0;
        }
    }
    wifi_hal_dbg_print("%s:%d Exit\n",__func__,__LINE__);
...
int platform_recreate_interface(wifi_radio_info_t *radio, wifi_vap_info_t *vap,
                                             wifi_interface_info_t **interface, bool create_mld)
{
    ...
    nl80211_remove_from_ovs_bridge(ifname);
    snprintf(cmd, sizeof(cmd), "wlanconfig %s destroy", ifname);
    wifi_hal_info_print("%s:%d: Executing %s\n", __func__, __LINE__, cmd);
    system(cmd);

    memset(cmd, 0, sizeof(cmd));
    snprintf(cmd, sizeof(cmd), "iw dev | grep -i mld%d", vap->vap_index);
    wifi_hal_info_print("%s:%d: Executing %s\n", __func__, __LINE__, cmd);

    snprintf(mld_ifname, sizeof(mld_ifname), "mld%d", vap->vap_index);
    if(system(cmd) == 0)
    {
        nl80211_remove_from_ovs_bridge(mld_ifname);
        memset(cmd, 0, sizeof(cmd));
        snprintf(cmd, sizeof(cmd), "iw dev mld%d del", vap->vap_index);
        wifi_hal_info_print("%s:%d: Executing %s\n", __func__, __LINE__, cmd);
        system(cmd);
    }
    else
    {
        wifi_hal_info_print("%s:%d: MLD not present. Not trying to delete it.\n", __func__, __LINE__);
    }

    if(create_mld)
    {
        snprintf(cmd, sizeof(cmd), "iw phy mld-phy0 interface add mld%d type __ap mld_addr " MACSTR,
                                                        vap->vap_index, MAC2STR(intf->mac));
        wifi_hal_info_print("%s:%d: Executing %s\n", __func__, __LINE__, cmd);
        system(cmd);
    }
    ...
    if (nla_put(msg, NL80211_ATTR_MAC, ETH_ALEN, intf->mac) < 0) {
        nlmsg_free(msg);
        return RETURN_ERR;
    }
    ...
    if(create_mld)
    {
        wifi_hal_dbg_print("%s:%d:Adding MLD addr " MACSTR " for vap index:%d ifname: %s\n", __func__, __LINE__,
                                                       MAC2STR(intf->mac), vap->vap_index, ifname);
        if (nla_put(msg, NL80211_ATTR_MLD_MAC, ETH_ALEN, intf->mac) < 0) {
            nlmsg_free(msg);
            wifi_hal_error_print("%s:%d: Error adding NL80211_ATTR_MLD_MAC to ifname: %s dev:%d\n",
                                                    __func__, __LINE__, ifname, radio->index);
            return RETURN_ERR;
        }
    }

    pthread_mutex_lock(&g_wifi_hal.hapd_lock);
    del_interface_by_vap_index(vap->vap_index);
    ...
    if ((intf = get_interface_by_vap_index(vap->vap_index)) != NULL) {
        wifi_hal_dbg_print("%s:%d:interface for vap index:%d updated\n", __func__, __LINE__,
            vap->vap_index);
    ...
    if(*interface == NULL)
    {
// This is platform_set_radio_pre_init case
        if(configure_hostap_interface(radio, intf, vap) != RETURN_OK)
        {
            wifi_hal_error_print("%s:%d: Failed to configure AP interface for VAP index %d.\n",
                                            __func__, __LINE__, vap->vap_index);
        }
    }
    pthread_mutex_unlock(&g_wifi_hal.hapd_lock);
