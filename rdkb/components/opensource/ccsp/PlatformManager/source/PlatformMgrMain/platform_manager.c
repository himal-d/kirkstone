...
#include "platform_manager_log.h"
...
PmThread_MsgItem pmThreadMsgArr[] = {
...
void PM_Util_get_shell_output( char * cmd, char *out, int len );
...
//static int pnm_inited = 0; //TODO: Need to Revisit
//static int netids_inited = 0; //TODO: Need to Revisit

//static int hotspot_started = 0; TODO //Need to Revisit
//static int lan_telnet_started = 0; TODO: //Need to Revisit
...
//static unsigned int factory_mode = 0; TODO: need to revisit
...
static void PMEthWan_EnterBridgeMode(void);
static void PMEthWan_EnterRouterMode(void);
/**************************************************************************/
/*      LOCAL FUNCTIONS:                                                  */
/**************************************************************************/

int checkIfUtopiaIsInstalled()
{
    if(!access("/tmp/utopia_inited", F_OK ))
    {
        return 1;
    }
    else
...
ePmThreadType Get_PmThreadType(char * name)
{
    errno_t rc       = -1;
    int     ind      = -1;
    ePmThreadType ret = PM_THREAD_ERROR;

    if (name != NULL && name[0] != '\0')
    {
        int i;
        for (i = 0; i < PM_THREAD_ERROR; i++) {
            rc = strcmp_s(pmThreadMsgArr[i].msgStr,strlen(pmThreadMsgArr[i].msgStr),name,&ind);
            ERR_CHK(rc);

            if((ind==0) && (rc == EOK))
            {
                ret = pmThreadMsgArr[i].mType;
                break;
            }
        }
    }
...
static int PmEthWan_SysCfgGetInt(const char *name)
{
   char out_value[20];
...
static int PMETHWAN_SysCfgSetInt(const char *name, int int_value)
{
   char value[20];

   snprintf(value,sizeof(value),"%d", int_value);
   PlatformManagerInfo((" %s : name = %s , value = %s \n", __FUNCTION__, name, value));
...
void validate_mode(int* bridge_mode)
{
    ...
    {
        PlatformManagerInfo((" SYSDB_CORRUPTION: bridge_mode = %d \n", *bridge_mode));
        PlatformManagerInfo((" SYSDB_CORRUPTION: Switching to Default Router Mode \n"));
        *bridge_mode = BRMODE_ROUTER;

        PMETHWAN_SysCfgSetInt("bridge_mode", *bridge_mode);
        if( syscfg_commit() != 0)
            PlatformManagerInfo((" %s : syscfg_commit not success \n", __FUNCTION__));

    }
    PlatformManagerInfo((" %s : bridge_mode = %d\n", __FUNCTION__, *bridge_mode));
 }
...
static void PMEthWan_EnterBridgeMode(void)
{
    ...
    char BridgeMode[2] = {0};
    PlatformManagerInfo((" Entry %s \n", __FUNCTION__));
    syscfg_get(NULL, "MoCA_current_status", MocaStatus, sizeof(MocaStatus));
    PlatformManagerInfo((" MoCA_current_status = %s \n", MocaStatus));
...
//    bridge_mode = 0;
    ...
    PlatformManagerInfo((" MocaPreviousStatus = %d \n", prev));
    if(prev == 1)
    {
...
{
    bridge_mode = PmEthWan_SysCfgGetInt("bridge_mode");
    active_mode = getSyseventBridgeMode(eRouterMode,bridge_mode);
}

static void PMEthWan_ProcessUtopiaRestart(void)
{
    ...
    PlatformManagerInfo((" bridge_mode = %d, active_mode = %d\n", bridge_mode, active_mode));
...
/**************************************************************************/
/*! \fn static STATUS PM_SysCfgGetInt
 **************************************************************************
 *  \brief Get Syscfg Integer Value
 *  \return int/-1
 **************************************************************************/
static int PM_SysCfgGetInt(const char *name)
{
   char out_value[20];
   int outbufsz = sizeof(out_value);
    PlatformManagerInfo((" %s : name = %s \n", __FUNCTION__, name));
   if (!syscfg_get(NULL, name, out_value, outbufsz))
   {
    PlatformManagerInfo((" value = %s \n", out_value));
      return atoi(out_value);
   }
   else
   {
    PlatformManagerInfo((" syscfg get failed \n"));
      return -1;
   }
}
...
static void check_lan_wan_ready()
{
    ...
    PlatformManagerInfo((" bridge-status = %s\n", br_st));
    PlatformManagerInfo((" lan-status = %s\n", lan_st));
    PlatformManagerInfo((" wan-status = %s\n", wan_st));
    PlatformManagerInfo((" ipv6_prefix = %s\n", ipv6_prefix));
    PlatformManagerInfo((" eRouterMode = %d\n", eRouterMode));
    if (eRouterMode == 2) {
        t2_event_d("SYS_INFO_ErouterMode2", 1);
    }
    PlatformManagerInfo((" bridge_mode = %d\n", bridge_mode));
...
/**************************************************************************/
/*! \fn void *PM_sysevent_threadfunc(void *data)
 **************************************************************************
 *  \brief Function to process sysevent event
 *  \return 0
**************************************************************************/
static void *PM_sysevent_threadfunc(void *data)
{
    ...
    PlatformManagerInfo(("Set notifications done \n"));    
    for (;;)
    {
/*#ifdef MULTILAN_FEATURE
...
/*#ifdef MULTILAN_FEATURE
        ...
        {
            PlatformManagerInfo((" %s : name = %s, val = %s \n", __FUNCTION__, name, val ));
            ePmThreadType ret_value;
            ethwan_enabled = IsEthWanEnabled();
            ret_value = Get_PmThreadType(name);
            ...
            {
                PlatformManagerInfo(("gw_prov_sm: got system restart\n"));                
                if (ethwan_enabled)
                {
                    PMEthWan_ProcessUtopiaRestart();
                }
                else
                {
            ...
            else if (ret_value == NTP_TIME_SYNC)
            {
                if (ethwan_enabled)
                {
                    PlatformManagerInfo(("ntp time syncd, need to restart sshd %s\n", name));
                    sysevent_set(sysevent_fd_gs, sysevent_token_gs, "sshd-restart", "", 0);
                }
            }
            else if (ret_value == FIREWALL_RESTART)            
            {
                if (ethwan_enabled)
                {
                    PlatformManagerInfo(("received notification event %s\n", name));
            ...
            else if ( (ret_value == PING_STATUS) || ( ret_value == CONN_STATUS ) )
            {
                ...
                {
                    PlatformManagerInfo(("Received ping-status event notification, ping-status value is %s\n", val));
                    rc = strcmp_s("missed", strlen("missed"),val, &ind);
                }
                else
                {
                    PlatformManagerInfo(("Received conn-status event notification, conn-status value is %s\n", val));
                ...
                if ((ind == 0) && (rc == EOK))
                {
                    ...
                    if ( ret_value == PING_STATUS )
                    {
                        PlatformManagerInfo(("Ping missed, Setting LED to WHITE FAST BLINK\n"));
                    }
                    else
                    {
                        PlatformManagerError(("Connection failed, Setting LED to WHITE FAST BLINK\n"));
                    }
#else
                    ...
                    if ( ret_value == PING_STATUS )
                    {
                        PlatformManagerInfo(("Ping missed, Setting LED to RED\n"));
                    }
                    else
                    {
                        PlatformManagerError(("Connection failed, Setting LED to RED\n"));
                    }
#endif
                    if(0 != platform_hal_setLed(&ledMgmt)) {

                        PlatformManagerError(("platform_hal_setLed failed\n"));
                    }
                ...
                else 
                {
                    ...
                    if ((ind == 0) && (rc == EOK))
                    {
                        ...
                        PlatformManagerInfo(("Default wan interface: '%s'.\n", defaultWanInterface));
                        ...
                        PlatformManagerInfo(("Current wan interface: '%s'.\n", currentWanInterface));
                        ...
                        if ((ind == 0) && (rc == EOK))
                        {
                          ...
                          PlatformManagerInfo(("Current wan interface is default one.\n"));
                          PlatformManagerInfo(("Setting LED: SOLID WHITE\n"));
                          ledMgmt.LedColor = WHITE;
                          ledMgmt.State  = SOLID;
                          ledMgmt.Interval = 0;
                        }
                        else {
                          PlatformManagerInfo(("Current wan interface is not default one.\n"));
                        }
                                    ...
                                    if ((ind == 0) && (rc == EOK))
                                    {
                                        if((responsefd = fopen(networkResponse, "r")) != NULL)
                                        {
                                            ...
                                            if ( 204 == iresCode )
                                            {
                                                ...
                                                if ((ind == 0) && (rc == EOK))
                                                {
                                                    PlatformManagerInfo(("NotifyWiFiChanges is true\n"));
                                                    ledMgmt.State    = BLINK;
                                                    ledMgmt.Interval = 1;
                                                }
                                            }
                        ...
                        {
                            PlatformManagerInfo(("Device is in Captive Portal, setting WHITE LED to blink\n"));
                        }
                        else
                        {
                            PlatformManagerInfo(("Device is not in Captive Portal, setting LED to SOLID WHITE \n"));
                        }
                        if(0 != platform_hal_setLed(&ledMgmt)) {
                            PlatformManagerError(("platform_hal_setLed failed\n"));
...
void PM_Util_get_shell_output( char * cmd, char *out, int len )
{
    ...
    if ( fp )
    {
        if (fgets( buf, sizeof( buf ), fp ) == NULL)
           PlatformManagerError(("%s fgets error \n", __FUNCTION__));
 ...
 *  \return 0
**************************************************************************/
static int Platform_Manager_Init()
{
    ...
    syscfg_init();
    if (0 != PM_SysCfgGetInt("bridge_mode"))
    {
        bridgeModeInBootup = 1;
    }
    bridge_mode = PM_SysCfgGetInt("bridge_mode");
    eRouterMode = PM_SysCfgGetInt("last_erouter_mode");
    ...
    if (sysevent_fd >= 0)
    {
        PlatformManagerInfo((" Creating Thread  PM_sysevent_threadfunc \n")); 
        pthread_create(&sysevent_tid, NULL, PM_sysevent_threadfunc, NULL);
    }
    memset(buf,0,sizeof(buf));
    ...
    if (0 == access( "/nvram/ETHWAN_ENABLE" , F_OK ))
    {
        if (syscfg_get(NULL, "eth_wan_enabled", buf, sizeof(buf)) == 0)
        {
            ...
            if((ind==0) && (rc == EOK))
            {
                ...
                {
                    PlatformManagerInfo(("eth_wan_enabled syscfg failed\n"));
                }
                else
                {
                    if (syscfg_commit() != 0)
                    {
                        PlatformManagerInfo(("eth_wan_enabled syscfg_commit\n"));
    ...
    else
    {
        ...
        {
            PlatformManagerInfo(("last wan mode is ethernet but nvram file not available !\n"));
            PlatformManagerInfo(("Update last wan mode as unknown\n"));
            snprintf(buf, sizeof(buf), "%d", WAN_MODE_UNKNOWN);
            if (syscfg_set(NULL, "last_wan_mode", buf) != 0)
            {
                PlatformManagerError(("last_wan_mode syscfg failed\n"));
            }
            else
            {
                if (syscfg_commit() != 0)
                {
                    PlatformManagerInfo(("last_wan_mode syscfg_commit\n"));
...

    {
        ...
        PlatformManagerError((" Failed to get EthWanInterfaceName: %s \n", ethwan_ifname ));
    }

    PlatformManagerInfo((" EthWanInterfaceName: %s \n", ethwan_ifname ));

#if defined (_BRIDGE_UTILS_BIN_) && !defined (_WNXL11BWL_PRODUCT_REQ_)
    if ( syscfg_set( NULL, "eth_wan_iface_name", ethwan_ifname ) != 0 )
    {
        PlatformManagerError(( "syscfg_set failed for eth_wan_iface_name\n" ));
    }
    else
    {
        if ( syscfg_commit() != 0 )
        {
            PlatformManagerError(( "syscfg_commit failed for eth_wan_iface_name\n" ));
        }
...
static void daemonize(void) {
	...
	case -1:
		// Error
		PlatformManagerError(("Error daemonizing (fork)! %s - %d\n", __FUNCTION__, __LINE__ ));
		exit(0);
		break;
	default:
		_exit(0);
...
int main(int argc, char *argv[])
{
   printf("Started gw_prov_utopia\n");
    ...
    t2_init("platformmanager");
    PlatformManagerLogInit();
    ...
    if(!checkIfUtopiaIsInstalled())
    {   
        PlatformManagerInfo((" Entry gw_prov_utopia\n"));
        PlatformManagerInfo((" Calling /etc/utopia/utopia_init.sh \n"));
        v_secure_system("/etc/utopia/utopia_init.sh");
    }
#endif

    Platform_Manager_Init();
    PlatformManagerInfo(("wait in loop \n"));
    while (1)
    {
        sleep(1);
    }
    ...
    if( findProcessId(argv[0]) > 0 )
    {
        ...
        PlatformManagerInfo((" gw_prov_utopia already running. Returning...\n"));
        return 1;
    }
