...
extern "C" pErr Process_Cellularmgr_WebConfigRequest(void *Data)
{
        if(!g_CellularAPIMock)
        {
                return 0;
        }
        return g_CellularAPIMock->Process_Cellularmgr_WebConfigRequest(Data);
}

extern "C" void freeResources_CELL(void *arg)
{
        if(!g_CellularAPIMock)
        {
                return;
        }
        return g_CellularAPIMock->freeResources_CELL(arg);
}
