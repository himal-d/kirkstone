...
{
        memcpy(ctx,gpstCellularPolicyCtrl,sizeof(CellularMgrPolicyCtrlSMStruct));
}
...
{   
...
#ifndef LTE_USB_FEATURE_ENABLED
       v_secure_system("ifconfig wwan0 %s netmask %s mtu %d up", pstIPStruct->IPAddress,pstIPStruct->SubnetMask,pstIPStruct->MTUSize);
#endif
        if(sysevent_fd >= 0)
        {
...
#ifndef LTE_USB_FEATURE_ENABLED
       v_secure_system("ifconfig wwan0 %s mtu %d up", pstIPStruct->IPAddress,pstIPStruct->MTUSize);
#endif
        if(sysevent_fd >= 0)
        {
    ...
    if( CELLULAR_NETWORK_IP_FAMILY_IPV4 == pstIPStruct->IPType )
    {
        ...
        CcspTraceInfo(("%s %d: enabling forwarding on v4\n", __FUNCTION__, __LINE__));
#ifndef LTE_USB_FEATURE_ENABLED
        system("conntrack -F"); 
        sleep(1);
        system("sysctl -w net.ipv4.conf.brWAN.forwarding=1 ; conntrack -F");
#endif
    ...
    //Set associated sysevents
#ifndef LTE_USB_FEATURE_ENABLED
        CcspTraceInfo(("%s %d: enabling forwarding on v6\n", __FUNCTION__, __LINE__));
        sleep(1);
        system("sysctl -w net.ipv6.conf.brWAN.forwarding=1");
#endif
...
#ifndef LTE_USB_FEATURE_ENABLED
    v_secure_system("ip link set dev wwan0 down");
#endif 
...
static CellularPolicySmState_t TransitionConnectedStopNetwork( void )
{    
    CcspTraceInfo(("%s %d: Link status to DOWN for %s\n", __FUNCTION__, __LINE__,gpstCellularPolicyCtrl->acWANIfName));

#ifndef LTE_USB_FEATURE_ENABLED
    v_secure_system("ip link set dev wwan0 down");

    CcspTraceInfo(("%s %d: Flush both IPv4 & IPv6\n", __FUNCTION__, __LINE__));
    v_secure_system("ip addr flush dev wwan0");
    v_secure_system("ip -6 addr flush dev wwan0");

    CcspTraceInfo(("%s %d: disabling forwarding\n", __FUNCTION__, __LINE__));
    system("sysctl -w net.ipv4.conf.brWAN.forwarding=0 ;sysctl -w net.ipv6.conf.brWAN.forwarding=0");
#endif 

    CcspTraceInfo(("%s %d: Flush the interface list\n", __FUNCTION__, __LINE__));
...
static CellularPolicySmState_t StateDeactivated( void )
{
    ...
    if ( (TRUE == gpstCellularPolicyCtrl->bRDKEnable) && 
         ((gpstCellularPolicyCtrl->enDeviceSlotSelectionStatus == DEVICE_SLOT_STATUS_SELECTING) ||
          (gpstCellularPolicyCtrl->enDeviceSlotSelectionStatus == DEVICE_SLOT_STATUS_READY)) )
    {
...
static ANSC_STATUS CellularMgr_ControllerInit( CellularMgrSMInputStruct    *pstInput )
{
    ...
    if( NULL != gpstCellularPolicyCtrl )
    {
        ...
        gpstCellularPolicyCtrl->bIPv6NetworkStartInProgress        = FALSE;
        gpstCellularPolicyCtrl->bIPv6WaitingForPacketStatus        = FALSE;
        snprintf(gpstCellularPolicyCtrl->acWANIfName, sizeof(gpstCellularPolicyCtrl->acWANIfName), "%s", CELLULAR_DEFAULT_IFNAME);
