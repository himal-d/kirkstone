...
static rbusError_t DeviceManagement_GetParamBoolValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts)
{
    ...
    else if(strcmp(context.name, "RebootDevice") == 0)
    {
        rbusProperty_SetBoolean(property, 0);
    }
    else
...
rbusError_t DeviceManagement_SetParamBoolValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusSetHandlerOptions_t* opts)
{
    ...
    if(strcmp(context.name, "FactoryReset") == 0)
    {
        ...
        if(val == false)
        {
                CcspTraceError(("%s-%d : false is wrong value\n",__FUNCTION__, __LINE__));
                return RBUS_ERROR_INVALID_INPUT;
        }
	...
	if(val == false)
	{
        ...
        CcspTraceInfo(("%s-%d : Cellular Modem Reset/Reboot...\n",__FUNCTION__, __LINE__));
        CellularMgr_RebootDevice();
...
static rbusError_t Firmware_GetParamStringValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts)
{
    ...
    {
        if( RETURN_OK == CellularMgr_GetModemFirmwareVersion(pstDmlCellular->CurrentImageVersion))
        {
...
static rbusError_t Cellular_Interface_GetParamBoolValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts)
{
    ...
    else if(strcmp(context.name, "X_RDK_PhyConnectedStatus") == 0)
    {
        ...
        CellularPolicySmState_t smState = CELLULAR_STATE_DOWN;
#ifndef CELLULAR_MGR_LITE
        smState = CellularMgrSMGetCurrentState( );
#endif
...
static rbusError_t Cellular_Interface_GetParamUlongValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts)
{
    ...
    else if(strcmp(context.name, "X_RDK_GlobalCellId") == 0)
    {
        CELL_LOCATION_SUBINFO loc = CellularMgr_GetCellLocationSubsciptionStatus( );
        CellularMgr_CellLocationInfo(pstInterfaceInfo,loc);
        rbusProperty_SetUInt32(property, pstInterfaceInfo->Global_cell_id);
    }
    else if(strcmp(context.name, "X_RDK_ServingCellId") == 0)
    {
        CELL_LOCATION_SUBINFO loc = CellularMgr_GetCellLocationSubsciptionStatus( );
        CellularMgr_CellLocationInfo(pstInterfaceInfo,loc);
        rbusProperty_SetUInt32(property, pstInterfaceInfo->Serving_cell_id);
    }
    else if(strcmp(context.name, "X_RDK_BandInfo") == 0)
    {
        CELL_LOCATION_SUBINFO loc = CellularMgr_GetCellLocationSubsciptionStatus( );
        CellularMgr_CellLocationInfo(pstInterfaceInfo,loc);
        rbusProperty_SetUInt32(property, pstInterfaceInfo->BandInfo);
    }
    else
    {
        return RBUS_ERROR_INVALID_INPUT;
...
static rbusError_t Cellular_Interface_GetParamStringValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts)
{
    ...
    else if(strcmp(context.name, "X_RDK_RadioEnvConditions") == 0)
    {
        ...
        CellularMgr_GetRadioEnvConditions( pstServingInfo, signal );

        if (pstServingInfo->RadioEnvConditions == RADIO_ENV_CONDITION_EXCELLENT)
        {
            rbusProperty_SetString(property, "EXCELLENT");
        }
        else if (pstServingInfo->RadioEnvConditions == RADIO_ENV_CONDITION_GOOD)
        {
            rbusProperty_SetString(property, "GOOD");
        }
        else if (pstServingInfo->RadioEnvConditions == RADIO_ENV_CONDITION_FAIR)
        {
            rbusProperty_SetString(property, "FAIR");
        }
        else if (pstServingInfo->RadioEnvConditions == RADIO_ENV_CONDITION_POOR)
        {
            rbusProperty_SetString(property, "POOR");
        }
        else if (pstServingInfo->RadioEnvConditions == RADIO_ENV_CONDITION_UNAVAILABLE)
        {
...
static rbusError_t SpeedTest_GetParamBoolValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusGetHandlerOptions_t* opts)
{
    ...
    if(strcmp(context.name, "Enable") == 0)
    {
        rbusProperty_SetBoolean(property, pstDmlCellular->X_RDK_SpeedTest_Enable);
    }
    else
    {
        return RBUS_ERROR_INVALID_INPUT;
...
rbusError_t SpeedTest_SetParamBoolValue_rbus(rbusHandle_t handle, rbusProperty_t property, rbusSetHandlerOptions_t* opts)
{
    HandlerContext context = GetPropertyContext(property);
    PCELLULARMGR_CELLULAR_DATA  pMyObject      =  (PCELLULARMGR_CELLULAR_DATA) g_pBEManager->hCellular;
    PCELLULAR_DML_INFO          pstDmlCellular =  (PCELLULAR_DML_INFO) pMyObject->pstDmlCellular;
    ...
    if(strcmp(context.name, "Enable") == 0)
    {
        ...
        if(val != pstDmlCellular->X_RDK_SpeedTest_Enable)
        {
            CellularMgr_EnableSpeedTest(val);
            pstDmlCellular->X_RDK_SpeedTest_Enable = val;
        }
        return RBUS_ERROR_SUCCESS;
    }
    else
