    ...
    )
{
	...
	if( bValue == false )
	{
		CcspTraceInfo (("false is wrong value\n"));
		return FALSE;
	}
    ...
    if( AnscEqualString(ParamName, "RebootDevice", TRUE))
    {
        ...
        CcspTraceInfo (("Cellular Modem RebootDevice...\n"));
...
{    
    ...
    if( AnscEqualString(ParamName, "X_RDK_RAT", TRUE))   
    {
        *puLong = pstServingInfo->Rat;
        return TRUE;
    }

    if( AnscEqualString(ParamName, "X_RDK_RFCN", TRUE))   
    {
    ...
    {
        CellularMgr_GetRadioEnvConditions( pstServingInfo, SIGNAL_NO_SUB_HAL_VALUE );
        *puLong = pstServingInfo->RadioEnvConditions;
        return TRUE;
    }
    
    if( AnscEqualString(ParamName, "X_RDK_GlobalCellId", TRUE))   
    {
        CellularMgr_CellLocationInfo( pstInterfaceInfo, LOC_NO_SUB_HAL_VALUE );
       *puLong = pstInterfaceInfo->Global_cell_id;
        return TRUE;
    }
    
        if( AnscEqualString(ParamName, "X_RDK_ServingCellId", TRUE))   
    {
        CellularMgr_CellLocationInfo( pstInterfaceInfo, LOC_NO_SUB_HAL_VALUE );      
        *puLong = pstInterfaceInfo->Serving_cell_id;
        return TRUE;
    }
    
        if( AnscEqualString(ParamName, "X_RDK_BandInfo", TRUE))   
    {
        CellularMgr_CellLocationInfo( pstInterfaceInfo, LOC_NO_SUB_HAL_VALUE );    
        *puLong = pstInterfaceInfo->BandInfo;
        return TRUE;
    }
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "X_RDK_CellId", TRUE))
    {
        /* collect value */
    ...
    if( AnscEqualString(ParamName, "X_RDK_PlmnId", TRUE))
    {
        /* collect value */
    ...
    if( AnscEqualString(ParamName, "X_RDK_AreaCode", TRUE))
    {
        /* collect value */
...
/**********************************************************************
    ...
    prototype:
        ...
        BOOL
        Cellular_Interface_GetParamIntValue
            (   
                ANSC_HANDLE                 hInsContext,
                char*                       ParamName,
                int*                        pInt
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "X_RDK_SNR", TRUE))
    {
    ...
    if( AnscEqualString(ParamName, "X_RDK_TRX", TRUE))
    {
    ...
    )
{
    ...
    {
        /* always return false when get */
        *pBool = pstDmlCellular->X_RDK_SpeedTest_Enable;
        return TRUE;
    }
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        CellularMgr_EnableSpeedTest(bValue);
        pstDmlCellular->X_RDK_SpeedTest_Enable = bValue;
        return TRUE;
    }
