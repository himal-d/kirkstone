...
#include "ledmgr_plugin_main_apis.h"
...
ANSC_HANDLE BackEndManagerCreate(VOID)
{
    ...
    LED_DATA*                       pMyObject    = (LED_DATA*)NULL;

    /*
     * We create object by first allocating memory for holding the variables and member functions.
     */
    pMyObject = (PLED_DATA)AnscAllocateMemory(sizeof(LED_DATA));
    if ( pMyObject == NULL )
    {
        return  (ANSC_HANDLE)NULL;
    }

    pMyObject->data = (char*)malloc(1000);
    if ( pMyObject->data == NULL )
    {
...
{
    ANSC_STATUS                     returnStatus = ANSC_STATUS_SUCCESS;
    LED_DATA*                       pMyObject    = (LED_DATA*)hThisObject;
    ...
    memset(pMyObject->data, 0, sizeof(char));
// #ifdef _COSA_SIM_
...
// #endif
    ...
    if(LedMgr_Rbus_Init() != ANSC_STATUS_SUCCESS)
    {
